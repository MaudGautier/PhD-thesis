...
% since each sequenced individual provides information for only one meiosis, the number of events per indivi

% are well suited to studies on species presenting both a short generation time and a large progeny, like yeasts \citep{mancera2008highresolution}, but do not scale well for mammals.
% Indeed, studies that have used such methods in humans require a large number of individuals but result in only a limited amount (a few hundreds at best) of detected recombination events \citep{williams2015noncrossover, halldorsson2016rate}.
%
...
% This demonstrated that, overall, there was no large capture bias among hotspots.
% But a few of them nevertheless displayed a haplotype bias, most likely because one of the two baits better matched one of the two haplotypes in certain hotspots.
...
% Brought back to the number of fragments\footnote{Fragments overlapping at least 4 markers, i.e.\ candidate recombination events.} mapping on these targets, this corresponded to a FP rate of 123\% \textbf{VOIR avec Laurent si plus de 100\% est problematique}.
% In other words, 98\% of all events on the B6 genome were not validated when using the CAST genome as a reference and thus likely corresponded to FPs.
% >>> 1088237/176039928
