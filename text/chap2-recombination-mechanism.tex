...
\begin{sidewaysfigure}[p]
	...
	{\textbf{Chromosome organisation diagrams and cytological pictures of the two meiotic divisions leading to the formation of four gametes.}
		...
		\par \mccorrect{The micrographs show meiosis in a male lily while the diagrams show it in an animal cell.}
		This figure was reproduced from \citet{hillis2012principles} (permission in Appendix~\ref{app:permissions}).
	}
...
But the prognosis regarding the equal number of COs and NCOs was never confirmed biologically \citep{bishop2004early} which \mccorrect{suggested that a portion of NCOs were created \textit{via} another mechanism.}
...
\begin{figure}[p]
	...
	{\textbf{Proteins involved in mammalian meiotic recombination.}
		...
		\textbf{d |} Recombination intermediates are either dismantled by BLM-RMI1-TOP3 to generate non-crossover intermediates, or stabilized by TEX11, MSH4-MSH5, RNF212, ZIP2, HFM1 and HEI10 to generate double Holliday junctions (CO intermediates).
...
\begin{figure}[p]
	...
	{\textbf{Molecular mechanism of pathways leading to crossing-overs (COs) and non-crossovers (NCOs).}
		...
		\mccorrect{This junction is then either dissolved into a NCO (left panel), resolved by canonical Holliday junction resolvases introducing a pair of symmetrical nicks to generate nicked DNA duplexes that can be directly ligated (middle panel) or resolved by noncanonical resolvases introducing asymmetrical nicks to produce gapped and flapped DNA duplexes that require further processing prior to ligation (bottom right panel).
	If only two strands are cleaved, the outcome is necessarily a NCO while it is a CO if all four strands are cleaved.}
...
In mice, a pair of \mccorrect{nicks} is introduced across the helical branchpoint of most (90\%) dHJs by the concerted action of the MLH1-MLH3 heterodimer \citep{baker1996involvement, edelmann1996meiotic,lipkin2002meiotic} and of EXO1 \citep{wei2003inactivationa}.
...
\begin{mccorrection}
In contrast, non-programmed DSBs, which correspond to DNA lesions, can be repaired either by homologous recombination \citep[reviewed in][]{sung2006mechanism} or by alternative processes.
% are repaired through different processes \citep[reviewed in][]{sung2006mechanism}.
Indeed, such spontaneous DSBs are frequent in mitotic cells and mitotic breaks are mainly repaired by recombining with the genetically identical sister chromatid, or \textit{via} one of two repair systems that are more error-prone \citep{smith2001influence}: non-homologous end-joining (NHEJ), which consits in directly ligating the broken strands of DNA \citep{weterings2004mechanism} or single-strand annealing (both reviewed in \citealp{helleday2003pathways} and \citealp{moynahan2010mitotic}).
\end{mccorrection}
