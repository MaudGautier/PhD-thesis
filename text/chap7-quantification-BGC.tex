...
Conversely in hotspots predicted to be targeted by PRDM9\textsuperscript{Cst}, most (72.44\%) haplotypes predicted to be hot contained a \textit{Prdm9\textsuperscript{Cst}} motif.
\end{mccorrection}
...
Taking into account the number of fragments in those regions, the NCO-1 FP error rate was as high as as 90.0\% (Table~\ref{tab:NCO-1-FP-rate}), which meant that as few as 14,766 of the pot-NCO-1 events detected within hotspots corresponded to genuine NCO-1 events.
...
More precisely, sequencing errors, by themselves, were expected to lead to a $\frac{SW}{WS+SW}$ ratio of 0.39 (see Appendix~\ref{app:data-and-figs}).

For pot-NCO-1 discoveries located within control regions, we found that the $\frac{SW}{WS+SW}$ ratio was 0.31. 
